...
import { listMyBookings } from "@/lib/booking/booking.functions";
...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your private members' diary, rewards and grooming history.",
      },
    ],
  }),
  component: () => (
    ...
    <div className="min-h-screen bg-ivory">
      <header className="border-b border-border bg-ivory/95">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 md:px-10">
          ...
          <div className="flex items-center gap-4 text-[11px] uppercase tracking-[0.18em]">
            <Link to="/booking" className="rounded-full bg-charcoal px-5 py-2.5 text-ivory">
              Book a service
            </Link>
            <button
              onClick={() => signOut()}
              className="hidden text-muted-foreground hover:text-charcoal sm:block"
            >
              Sign out
            </button>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-5 py-10 md:px-10 md:py-16">
        ...
        <p className="mt-3 max-w-xl text-muted-foreground">
          Manage your upcoming visits and view your booking history in one place.
        </p>
        <section className="mt-12">
          <h2 className="mb-5 font-display text-2xl text-charcoal">Upcoming</h2>
          <MyAppointments />
        </section>
        <section className="mt-16">
          <h2 className="mb-5 font-display text-2xl text-charcoal">Booking history</h2>
...
function LaunchBookingHistory() {
  const listFn = useServerFn(listMyBookings);
  const bookingsQ = useQuery({
    queryKey: ["my-bookings"],
    queryFn: async () => listFn({ data: { accessToken: await dashboardToken() } }),
  });
  const past = (bookingsQ.data ?? []).filter(
    (booking) => new Date(booking.scheduled_at).getTime() < Date.now(),
  );

  if (bookingsQ.isLoading)
    return <p className="text-sm text-muted-foreground">Loading your history…</p>;
  if (bookingsQ.isError)
    return <p className="text-sm text-destructive">{(bookingsQ.error as Error).message}</p>;
  if (past.length === 0)
    return (
    ...
    <div className="overflow-x-auto border-y border-border">
      <table className="w-full min-w-[620px] text-sm">
        ...
        <tbody className="divide-y divide-border">
          {past.map((booking) => (
            <tr key={booking.id}>
              <td className="py-4 font-display text-lg text-charcoal">{booking.service_name}</td>
              <td className="text-muted-foreground">
                {new Date(booking.scheduled_at).toLocaleDateString("en-GB")}
              </td>
              <td className="text-muted-foreground">{booking.duration_minutes} min</td>
              <td className="capitalize text-muted-foreground">
                {booking.status.replace("_", " ")}
              </td>
              <td className="text-right font-display text-charcoal">
                £{(booking.price_pence / 100).toFixed(booking.price_pence % 100 === 0 ? 0 : 2)}
              </td>
              <td className="text-xs uppercase tracking-[0.14em] text-gold">
                {booking.reference ?? "—"}
              </td>
            </tr>
          ))}
    ...
    <div className="min-h-screen bg-ivory">
      ...
      <div className="flex">
        ...
        <aside className="hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 lg:left-0 lg:w-72 border-r border-border bg-cream">
          <SidebarContent section={section} onSelect={(s) => setSection(s)} unread={unread} />
        </aside>
        ...
        {mobileNavOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            />
            <aside
              className="absolute inset-y-0 left-0 w-80 max-w-[85%] bg-cream shadow-luxe animate-slide-in-right"
        ...
        <main className="flex-1 lg:pl-72 pb-24 lg:pb-0">
          <div
            className="mx-auto max-w-7xl px-5 py-8 md:px-10 md:py-12 animate-fade-in"
            key={section}
          >
            ...
            return (
              <li key={item.id}>
                ...
                >
                  ...
                  <Icon
                    className={`h-4 w-4 ${active ? "text-gold" : "text-charcoal/60"}`}
                    strokeWidth={1.5}
                  />
                  <span className="flex-1 text-left">{item.label}</span>
                  {item.id === "notifications" && unread > 0 && (
                    <span className="rounded-full bg-charcoal px-2 py-0.5 text-[10px] text-ivory">
                      {unread}
                    </span>
                  )}
                </button>
...
function Overview({ onNavigate }: { onNavigate: (s: SectionId) => void }) {
  ...
  const progress = Math.min(
    100,
    Math.round(((member.points - currentTier.min) / (currentTier.max - currentTier.min)) * 100),
  );
      ...
      <header className="flex flex-wrap items-end justify-between gap-6">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            A quiet glance over your diary, rewards and rituals.
          </p>
        </div>
        <Link
          to="/booking"
          className="hidden md:inline-flex rounded-full bg-charcoal px-6 py-3 text-[12px] tracking-[0.2em] uppercase text-ivory hover:opacity-90"
        >
          Book your chair
        </Link>
      </header>
          ...
          <div className="relative">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-3">
              ...
              <Meta
                ...
                label="Time"
                value={`${nextAppointment.time} · ${nextAppointment.duration}`}
              />
              <Meta
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <button className="rounded-full bg-ivory px-5 py-2.5 text-[11px] tracking-[0.2em] uppercase text-charcoal">
                View details
              </button>
          ...
          <ul className="divide-y divide-border">
            {upcoming.map((a) => (
              <li key={a.id} className="grid gap-2 py-4 md:grid-cols-12 md:items-center">
                ...
                <p className="text-sm text-charcoal md:col-span-4">
                  {a.shortDate} · {a.time}
                </p>
          ...
          <ul className="space-y-4">
            {notifs.slice(0, 3).map((n) => (
              <li key={n.id} className="flex gap-3">
                <span
                  className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${n.unread ? "bg-gold" : "bg-border"}`}
                />
                <div>
    ...
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <Link
            to="/booking"
            className="rounded-full bg-charcoal px-5 py-2.5 text-[11px] tracking-[0.2em] uppercase text-ivory hover:opacity-90"
          >
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        subtitle={`${history.length} visits since ${member.memberSince}.`}
      />
            ...
            {rows.map((h) => (
              <tr key={h.id} className="transition hover:bg-cream/40">
                ...
                <td className="px-6 py-4">
                  <div className="flex gap-0.5">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`h-3.5 w-3.5 ${i < h.rating ? "text-gold" : "text-border"}`}
                        fill="currentColor"
                      />
                    ))}
                  </div>
                ...
                <td className="px-6 py-4 text-right">
                  <button className="text-[11px] tracking-[0.18em] uppercase text-charcoal hover:text-gold">
                    Rebook →
                  </button>
                </td>
              </tr>
...
function LoyaltyClub() {
  ...
  const progress = Math.min(
    100,
    Math.round(((member.points - current.min) / (current.max - current.min)) * 100),
  );

  return (
    <div className="space-y-10">
      <SectionHeader
        eyebrow="The Glow Circle"
        title="Loyalty Club"
        subtitle="A quieter way to belong."
      />
        ...
        <div className="relative grid gap-10 md:grid-cols-2 md:items-center">
          <div>
            ...
            <p className="mt-4 text-ivory/75">
              You are {member.pointsToNextTier} points from your next standing.
            </p>
            <div className="mt-8">
          ...
          <ul className="space-y-3">
            {current.perks.map((p) => (
              <li
                key={p}
                className="flex items-start gap-3 border-b border-ivory/10 pb-3 last:border-0"
              >
                <Check className="mt-0.5 h-4 w-4 text-champagne" />{" "}
                <span className="text-ivory/90">{p}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        {tiers.map((t, i) => {
          ...
          return (
            ...
            >
              ...
              <Crown
                className={`h-7 w-7 ${isCurrent ? "text-gold" : "text-charcoal/40"}`}
                strokeWidth={1.2}
              />
              ...
              <ul className="mt-5 space-y-2.5 text-sm text-charcoal">
                {t.perks.map((p) => (
                  <li key={p} className="flex items-start gap-2">
                    <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold" /> {p}
                  </li>
                ))}
              </ul>
            </article>
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        eyebrow="Promotions"
        title="Your Coupons"
        subtitle="Member-only offers, ready to use at checkout."
      />
              ...
              <div className="relative bg-card p-6 -m-1">
                ...
                <div className="mt-5 flex items-end justify-between gap-4">
                  <div>
                    ...
                    <code className="font-mono text-lg tracking-[0.2em] text-charcoal">
                      {c.code}
                    </code>
                  </div>
    ...
    <div className="space-y-8">
      ...
      <ul className="divide-y divide-border bg-card shadow-soft">
        {rows.map((n) => (
          <li
            key={n.id}
            className={`group flex items-start gap-5 p-6 transition hover:bg-cream/40 ${n.unread ? "" : "opacity-75"}`}
          >
            <span
              className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${n.unread ? "bg-gold" : "bg-border"}`}
            />
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-3">
                <span className="rounded-full bg-cream px-2.5 py-0.5 text-[10px] tracking-[0.18em] uppercase text-charcoal">
                  {n.category}
                </span>
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        subtitle="Re-book the rituals you return to."
      />
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {favouriteServices.map((f) => (
          <article
            key={f.id}
            className="group flex flex-col bg-card p-7 shadow-soft transition hover:shadow-luxe"
          >
            ...
            <Link
              to="/booking"
              className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-charcoal py-2.5 text-[11px] tracking-[0.2em] uppercase text-ivory hover:opacity-90"
            >
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        eyebrow="Account"
        title="Profile"
        subtitle="Your details, communication and marketing preferences."
      />
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <Panel title="Personal details">
            ...
            <div className="mt-7 flex justify-end gap-3">
              <button className="rounded-full border border-border px-5 py-2.5 text-[11px] tracking-[0.2em] uppercase text-charcoal hover:border-charcoal">
                Cancel
              </button>
              <button className="rounded-full bg-charcoal px-6 py-2.5 text-[11px] tracking-[0.2em] uppercase text-ivory hover:opacity-90">
                Save changes
              </button>
            </div>
          </Panel>
            ...
            <ul className="divide-y divide-border">
              {(["email", "whatsapp", "sms", "post"] as const).map((k) => (
                <li key={k} className="flex items-center justify-between py-4">
                  <div>
                    <p className="font-display text-charcoal capitalize">
                      {k === "sms" ? "SMS" : k}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Receive {k === "post" ? "hand-pressed mail" : `${k} updates`} about your
                      bookings and rewards.
                    </p>
                  </div>
                  <Toggle
                    on={marketing[k]}
                    onChange={() => setMarketing({ ...marketing, [k]: !marketing[k] })}
                  />
                </li>
              ))}
            </ul>
        ...
        <div className="space-y-6">
          <div className="bg-charcoal p-8 text-ivory shadow-luxe">
            ...
            <p className="mt-1 text-ivory/70 text-sm">
              {member.tier} · No. 00{member.lifetimeVisits}284
            </p>
            <div className="gold-rule mt-5" />
            <p className="mt-5 text-[11px] tracking-[0.2em] uppercase text-ivory/60">
              Member since
            </p>
          ...
          <Panel title="Security">
            <button className="w-full rounded-full border border-border py-2.5 text-[11px] tracking-[0.2em] uppercase text-charcoal hover:border-charcoal">
              Change password
            </button>
            <button className="mt-3 w-full rounded-full border border-border py-2.5 text-[11px] tracking-[0.2em] uppercase text-charcoal hover:border-charcoal">
...
function ReferAndEarn() {
  ...
  return (
    <div className="space-y-10">
      <SectionHeader
        eyebrow="Refer & Earn"
        title="Share the Circle"
        subtitle="£25 credit for you. £25 for them."
      />
          ...
          <tbody className="divide-y divide-border">
            {referrals.map((r) => (
              <tr key={r.name}>
                ...
                <td className="py-4">
                  <span
                    className={`rounded-full px-3 py-1 text-[10px] tracking-[0.18em] uppercase ${r.status === "Joined" ? "bg-cream text-charcoal" : "bg-cream/50 text-muted-foreground"}`}
                  >
                    {r.status}
                  </span>
                </td>
  ...
  return (
    <div className="space-y-10">
      <SectionHeader
        eyebrow="We're here"
        title="Help & Support"
        subtitle="A concierge is one tap away."
      />
      ...
      <div className="grid gap-5 md:grid-cols-3">
        ...
        <SupportCard
          icon={Phone}
          title="Call us"
          sub={SALON_CONFIG.phone || "+44 20 7000 0184"}
          cta="Call now"
        />
      </div>
      ...
      <div className="bg-card p-8 shadow-soft md:p-10">
        ...
        <div className="mt-5">
          ...
          <textarea
            rows={4}
            className="mt-2 w-full rounded-md border border-border bg-cream/40 px-4 py-3 text-charcoal outline-none focus:border-charcoal"
          />
        </div>
        <div className="mt-6 flex justify-end">
          <button className="rounded-full bg-charcoal px-7 py-3 text-[11px] tracking-[0.2em] uppercase text-ivory hover:opacity-90">
            Send
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-card p-7 shadow-soft transition hover:shadow-luxe">
      ...
      <p className="mt-5 font-display text-4xl text-charcoal">
        {value}
        {unit && (
          <span className="ml-2 text-sm font-sans uppercase tracking-[0.18em] text-muted-foreground">
            {unit}
          </span>
        )}
      </p>
    ...
    <div className={`bg-card p-6 shadow-soft md:p-8 ${className}`}>
      <div className="flex items-center justify-between border-b border-border pb-4">
        ...
        {actionLabel && (
          <button
            onClick={onAction}
            className="text-[11px] tracking-[0.18em] uppercase text-charcoal hover:text-gold"
          >
            {actionLabel} →
          </button>
        )}
...
function Meta({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div>
      <div className="flex items-center gap-1.5 text-[10px] tracking-[0.2em] uppercase text-champagne">
        {icon} {label}
      </div>
...
function QuickAction({ label, onClick, to }: { label: string; onClick?: () => void; to?: string }) {
  const cls =
    "group flex w-full items-center justify-between border-b border-border py-3 text-sm text-charcoal transition hover:text-gold last:border-0";
  if (to)
    return (
      <li>
...
function StatusPill({ status, className = "" }: { status: string; className?: string }) {
  ...
  return (
    <span
      className={`inline-flex w-fit items-center gap-1.5 rounded-full px-3 py-1 text-[10px] tracking-[0.18em] uppercase ${map[status] || "bg-cream"} ${className}`}
    >
...
function Toggle({ on, onChange }: { on: boolean; onChange: () => void }) {
  return (
    ...
    >
      <span
        className={`absolute top-0.5 h-5 w-5 rounded-full bg-ivory shadow transition ${on ? "left-[22px]" : "left-0.5"}`}
      />
    </button>
  );
}
...
function SupportCard({
  ...
  return (
    <article
      className={`flex flex-col p-7 shadow-soft transition hover:shadow-luxe ${dark ? "bg-charcoal text-ivory" : "bg-card text-charcoal"}`}
    >
      ...
      <button
        className={`mt-6 self-start rounded-full px-5 py-2.5 text-[11px] tracking-[0.2em] uppercase ${dark ? "bg-ivory text-charcoal" : "bg-charcoal text-ivory"} hover:opacity-90`}
      >
        {cta}
      </button>
    </article>
