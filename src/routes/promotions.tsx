  ...
  head: () => ({
    meta: [
      { title: `Offers & Seasonal Rituals — ${SALON_CONFIG.name}` },
      {
        name: "description",
        content:
          "Seasonal offers, first-visit discounts and referral rewards at Lux & Glow, Ashford.",
      },
      { property: "og:title", content: `Offers — ${SALON_CONFIG.name}` },
      {
        property: "og:description",
        content: "Welcome offer, referral credit and seasonal deals at our Ashford unisex salon.",
      },
    ],
        ...
        <div className="mx-auto max-w-7xl px-5 py-20 md:px-10 md:py-24">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {seasonal.map((s) => (
              <article
                key={s.title}
                className="group flex flex-col bg-ivory p-8 shadow-soft transition hover:shadow-luxe"
              >
                ...
                <div className="mt-7 flex items-center justify-between border-t border-border pt-5">
                  ...
                  <Link
                    to="/booking"
                    className="text-[11px] tracking-[0.2em] uppercase text-charcoal group-hover:text-gold"
                  >
                    Reserve →
                  </Link>
                </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 md:px-10 md:py-28">
        <div className="grid gap-12 md:grid-cols-12">
          <div className="md:col-span-5">
            ...
            <Link
              to="/loyalty"
              className="mt-8 inline-block rounded-full bg-charcoal px-7 py-3.5 text-[12px] tracking-[0.2em] uppercase text-ivory hover:opacity-90"
            >
              Discover membership
            </Link>
          </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 md:grid-cols-2 md:px-10 md:py-28">
          <div>
            ...
            <p className="mt-4 max-w-md text-ivory/75">
              Hand-pressed envelopes, bicycle-delivered within Ashford. From a single shave to a
              year of cuts.
            </p>
          </div>
          <div className="flex items-end justify-start gap-4 md:justify-end">
            <Link
              to="/contact"
              className="rounded-full border border-champagne px-7 py-3.5 text-[12px] tracking-[0.2em] uppercase text-champagne hover:bg-champagne hover:text-charcoal"
            >
              Order a gift card
            </Link>
          </div>
...
function FeatureCard({
  ...
  return (
    <article
      className={`relative overflow-hidden p-10 md:p-12 ${dark ? "bg-charcoal text-ivory" : "bg-cream text-charcoal"}`}
    >
      <Icon className={`h-8 w-8 ${dark ? "text-champagne" : "text-gold"}`} strokeWidth={1.2} />
      <p
        className={`mt-6 text-[11px] tracking-[0.2em] uppercase ${dark ? "text-champagne" : "text-gold"}`}
      >
        {eyebrow}
      </p>
      ...
      <div
        className={`mt-8 flex items-center justify-between border-t pt-5 ${dark ? "border-ivory/15" : "border-border"}`}
      >
        <div>
