...
import { listServices, getAvailability, createBooking } from "@/lib/booking/booking.functions";
  ...
  head: () => ({
    meta: [
      { title: `Book Online — ${SALON_CONFIG.name} Barber & Salon, Ashford` },
      {
        name: "description",
        content:
          "Book a haircut, skin fade, beard trim, colour or beauty treatment at Lux & Glow in Ashford, TW15. Live availability, instant confirmation.",
      },
      { property: "og:title", content: `Book Online — ${SALON_CONFIG.name}` },
      {
        property: "og:description",
        content:
          "Choose your service, stylist and time — live availability at Lux & Glow, Ashford.",
      },
...
function BookingPage() {
  ...
  const availabilityQ = useQuery({
    ...
    enabled: !!serviceId,
    queryFn: () => availabilityFn({ data: { date, serviceId: serviceId!, stylistId: null } }),
  });
  ...
  const canConfirm =
    !!service && !!time && contactName.trim().length > 1 && contactPhone.trim().length > 6;
      ...
      <SiteShell>
        <section className="mx-auto max-w-2xl px-5 py-28 text-center md:px-10">
          ...
          <p className="mt-4 text-muted-foreground">
            <span className="text-charcoal">{b.serviceName}</span> on{" "}
            {prettyDate(b.scheduledAt.slice(0, 10))} at {b.scheduledAt.slice(11, 16)}.
          </p>
          <p className="mt-3 text-sm text-muted-foreground">
            {b.durationMinutes} minutes · {money(b.pricePence)}
          </p>
          <p className="mt-4 inline-block rounded-full bg-cream px-5 py-2 font-display text-lg text-charcoal">
            Reference {b.reference}
          </p>
          <p className="mt-4 text-sm text-muted-foreground">
            Your appointment is saved to your dashboard. Keep this reference for your records. Call
            us on {SALON_CONFIG.phone} for anything urgent.
          </p>
          <div className="mt-10 flex justify-center gap-3">
            <Link
              to="/dashboard"
              className="rounded-full bg-charcoal px-6 py-3 text-[12px] tracking-[0.2em] uppercase text-ivory"
            >
              My Appointments
            </Link>
            <Link
              to="/"
              className="rounded-full border border-charcoal/30 px-6 py-3 text-[12px] tracking-[0.2em] uppercase text-charcoal"
            >
              Return Home
            </Link>
          </div>
    ...
    <SiteShell>
      <section className="mx-auto max-w-6xl px-5 pt-16 pb-24 md:px-10 md:pt-24">
        ...
        <p className="mt-4 max-w-xl text-muted-foreground">
          Live availability at {SALON_CONFIG.name}, {SALON_CONFIG.seo.streetAddress}. Sign in to
          book and manage your appointment.
        </p>
        ...
        <div className="mt-12 grid gap-10 md:grid-cols-3">
          <div className="space-y-10 md:col-span-2">
            ...
            <Step n="03" label={`Select a time · ${prettyDate(date)}`}>
              {!serviceId ? (
          ...
          <aside className="self-start bg-cream p-7 md:sticky md:top-28">
            ...
            <div className="mt-6 flex items-center justify-between border-t border-border pt-5">
              ...
              <span className="font-display text-2xl text-charcoal">
                {service ? money(service.price_pence) : "—"}
              </span>
            </div>
            {confirm.isError && (
            ...
            <p className="mt-3 text-center text-[11px] text-muted-foreground">
              No charge today — settle in salon.
            </p>
          </aside>
        </div>
      </section>
...
function Empty({ text }: { text: string }) {
  return (
    <p className="border border-dashed border-border p-6 text-sm text-muted-foreground">{text}</p>
  );
}
